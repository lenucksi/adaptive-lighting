...
async def test_unique_config_entry_duplicate_name(hass):
    """Test that duplicate config names are rejected."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        title="unique_test",
        data={CONF_NAME: "unique_test"},
        options={},
    )
    entry.add_to_hass(hass)
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        user_input={CONF_NAME: "unique_test"},
    )
    assert result["type"] == FlowResultType.ABORT
    assert result["reason"] == "already_configured"


async def test_import_no_data(hass):
    """Test import step with no data."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN,
        context={"source": "import"},
        data=None,
...
async def test_import_updates_existing_entry(hass):
    """Test import step updates existing entry."""
    data = DEFAULT_DATA.copy()
    data[CONF_NAME] = DEFAULT_NAME
...
async def test_options_flow_with_invalid_lights(hass):
    """Test options flow with invalid light entities."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        title=DEFAULT_NAME,
        data={CONF_NAME: DEFAULT_NAME},
        options={"lights": ["light.nonexistent"]},
    )
    entry.add_to_hass(hass)
